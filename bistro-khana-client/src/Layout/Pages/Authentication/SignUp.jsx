        ...
        signUp(email, password)
            .then(user => {
                updateName(name, photo)
                    .then(() => {
                        const userinfo = { name: user.user.displayName, email: user.user.email}
                        ...
                        <div className="card-body inter-font ">
                            ...
                            <form onSubmit={handleSubmit(onSubmit)}>
                                <fieldset className="fieldset">
                                    ...
                                    <input className="input" {...register("name", { required: true, maxLength: 20 })} aria-invalid={errors.name ? "true" : "false"} placeholder='Enter your name here' />
                                    {errors.name?.type === "required" && (
                                        <p className='text-red-600 '>Name is required</p>)}
                                    {/* photo */}
                                    <label className="dark-2 text-xl font-semibold">Photo</label>
                                    <input className="input" {...register("photo")} aria-invalid={errors.photo ? "true" : "false"} placeholder='Enter you profile photo' />
